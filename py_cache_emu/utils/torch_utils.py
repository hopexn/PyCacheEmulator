...
from .mp_utils import manager

device = torch.device("cpu")
_mutex = mp.Lock()
_n_allocated = manager.Value("n_allocated", 0)
...
def register_device(**kwargs):
    ...
    if torch.cuda.is_available():
        with _mutex:
            gpu_id = (_n_allocated.get() + 1) % torch.cuda.device_count()
            device = torch.device("cuda:{}".format(gpu_id))
            _n_allocated.set(_n_allocated.get() + 1)
    else:
        device = torch.device("cpu")
